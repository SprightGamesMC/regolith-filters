...
const DebuggerLaunchConfig = require("./DebuggerLaunchConfig.js");
const EsbuildCompiler = require("./EsbuildCompiler.js");
const FilterLogger = require("./FilterLogger.js");
const FilterTiming = require("./FilterTiming.js");
const FilterPaths = require("./FilterPaths.js");
const ProjectExportResolver = require("./ProjectExportResolver.js");
const RuntimeSourceMapInjector = require("./RuntimeSourceMapInjector.js");
...
class TsCompileFilter {
  ...
  constructor(cwd, projectRoot, rawSettings) {
    ...
    this.debuggerLaunchManager = new DebuggerLaunchConfig({
      ...
      getActiveDistDir: () => this.getActiveDistDir(),
   ...
   *
   * @returns {Promise<void>} Resolves after post-build tasks complete.
   */
  async finalizeBuild() {
    if (this.settings.enableDebugger) {
      for (const compiledOutputPath of this.getDebuggerOutputPaths()) {
        ...
        const injectedSourceMapping = await this.runtimeSourceMaps.generateSourceMapping(
          compiledOutputPath,
          derivedOutputPath
        );
   ...
   * @param {Record<string, any>} rawSettings - Raw JSON settings passed to the filter.
   *
   * @returns {Record<string, any>} Merged filter settings.
   */
  createSettings(rawSettings) {
    const rawBuildOptions =
      rawSettings.buildOptions &&
      typeof rawSettings.buildOptions === "object" &&
      !Array.isArray(rawSettings.buildOptions)
        ? rawSettings.buildOptions
        : {};
    const defaults = {
      buildOptions: {
    ...
    settings.buildOptions = Object.assign({}, defaults.buildOptions, rawBuildOptions);
  ...
  ensureResolvedPaths() {
    ...
    const derivedOutputPath = this.settings.keepSource
      ? FilterPaths.joinRelativePath("dist", outputRelativePath)
      : outputRelativePath;
  ...
  loadConfigFile() {
    ...
    const config = loadedConfig && typeof loadedConfig.config === "function" ? loadedConfig.config : null;

    if (!config) {
      this.logger.warn(
        `Skipping "${TsCompileFilter.CONFIG_FILENAME}" because it must export a "config(settings)" function.`
      );
      return;
    }
   ...
   *
   * @throws {Error} Thrown when output path configuration is invalid.
   */
  ...
  validateSettings() {
    ...
    if (!this.isScriptEntryPath(this.settings.sourceEntry)) {
      throw new Error(
        `"sourceEntry" must use one of these script extensions: ${TsCompileFilter.SUPPORTED_SCRIPT_EXTENSION_LIST}.`
      );
    }
    ...
    if (!fs.existsSync(sourceEntryPath)) {
      throw new Error(
        `Could not find "sourceEntry" at "${this.settings.sourceEntry}" inside "${this.settings.sourceDir}".`
      );
    }
   ...
   *
   * @throws {TypeError} Thrown when the value is the wrong type.
   */
   ...
   * @param {string} expectedType - Expected type description.
   *
   * @throws {TypeError} Thrown when the value is present with the wrong type.
   */
   ...
   *
   * @throws {TypeError} Thrown when `debuggerProfile` is not a string.
   * @throws {Error} Thrown when `debuggerProfile` is missing or invalid.
   */
   ...
   *
   * @returns {{ entryPoint: string; hasCommonJs: boolean; hasJsonImport: boolean }[]} Cached entry analysis records.
   */
  analyzeSplitBuildEntries() {
    ...
    this.entrySourceAnalysis = this.settings.buildOptions.entryPoints.map((entryPoint) => {
   ...
   *
   * @throws {Error} Thrown when a split-build source file uses unsupported syntax.
   */
  ...
  applyExternalModules() {
    ...
    const externalModules = Array.isArray(this.settings.buildOptions.external)
      ? [...this.settings.buildOptions.external]
      : [];
   ...
   *
   * @returns {{ name: string; version: string }[]} Parsed module definitions.
   *
   * @throws {Error} Thrown when a module string is invalid.
   */
  parseModules() {
    ...
    this.parsedModules = this.settings.modules.map((moduleDefinition) => {
      const match =
        /^(@minecraft\/[A-Za-z0-9-]+)@((?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)(?:-[0-9A-Za-z.-]+)?)$/.exec(
          moduleDefinition
        );
   ...
   *
   * @throws {Error} Thrown when the manifest contains conflicting module data.
   */
  updateManifest() {
    ...
    for (const moduleDefinition of parsedModules) {
      const existingDependency = manifest.dependencies.find((dependency) => {
      ...
      if (existingDependency) {
        if (existingDependency.version !== moduleDefinition.version) {
          throw new Error(
            `BP/manifest.json already lists "${moduleDefinition.name}" with version "${existingDependency.version}", but tscompile was asked for version "${moduleDefinition.version}". Update one side so they match.`
          );
        }
    ...
    const existingModule = manifest.modules.find(
      (moduleDefinition) => moduleDefinition.type === TsCompileFilter.MODULE_TYPE
    );
   ...
   *
   * @returns {boolean} `true` when the source directory exists and should be removed.
   */
  ...
  getCompiledOutputPath() {
    this.ensureResolvedPaths();

    return this.resolvedPaths.compiledOutputPath;
  }
   ...
   *
   * @returns {string} Active distribution directory path.
   */
  getActiveDistDir() {
    this.ensureResolvedPaths();

    return this.resolvedPaths.activeDistDir;
  }

  /**
   * Derives the runtime JavaScript output path from the configured source entry.
   *
   * @returns {string} Derived output path relative to `BP/scripts`.
   */
  getDerivedOutputPath() {
    this.ensureResolvedPaths();

    return this.resolvedPaths.derivedOutputPath;
  }

  /**
   * Gets the compiled JavaScript outputs that need debugger post-processing.
   *
   * @returns {string[]} Absolute output file paths.
   */
   ...
   *
   * @param {string} compiledOutputPath - Absolute output file path.
   *
   * @returns {string} Output path relative to `BP/scripts`.
   *
   * @throws {Error} Thrown when the output is outside the scripts root.
   */
  getDerivedOutputPathForCompiledOutput(compiledOutputPath) {
    ...
    if (relativeOutputPath === "" || relativeOutputPath.startsWith("../") || path.isAbsolute(relativeOutputPath)) {
      throw new Error(
        `Expected a compiled output inside "${TsCompileFilter.SCRIPTS_ROOT}", but received "${compiledOutputPath}".`
      );
    }
   ...
   *
   * @param {string} compiledOutputPath - Absolute output file path.
   * @param {string} scriptsRootPath - Absolute `BP/scripts` root path for the current workspace.
   *
   * @returns {string} Output path relative to `BP/scripts`.
   */
  extractScriptsRelativeOutputPath(compiledOutputPath, scriptsRootPath) {
    ...
    return FilterPaths.normalizeRelativePath(
      normalizedCompiledOutputPath.slice(scriptsRootIndex + scriptsRootMarker.length)
    );
  }
   ...
   *
   * @returns {string} Project-relative source entry path.
   */
  getSourceEntryProjectPath() {
    this.ensureResolvedPaths();

    return this.resolvedPaths.sourceEntryProjectPath;
  }

  /**
   * Gets the absolute source directory path from the project root.
   *
   * @returns {string} Absolute source directory path.
   */
   ...
   *
   * @returns {string} Absolute source entry path.
   */
  getSourceEntryPath() {
   ...
   *
   * @returns {string} Absolute workspace source directory path.
   */
   ...
   *
   * @returns {string} Project-relative scripts root path.
   */
   ...
   *
   * @param {string} relativePath - Path relative to the local scripts root.
   *
   * @returns {string} Project-relative local path.
   */
  ...
  /**
   * Determines whether a path is a supported TypeScript source file.
   *
   * @param {string} relativePath - Relative path to validate.
   *
   * @returns {boolean} True when the path is a supported TypeScript source file.
   */
  isTypeScriptEntryPath(relativePath) {
    return (
      TsCompileFilter.TYPESCRIPT_ENTRY_PATTERN.test(relativePath) &&
      !TsCompileFilter.TYPESCRIPT_DECLARATION_PATTERN.test(relativePath)
    );
  }
   ...
   * @param {string} relativePath - Relative path to validate.
   *
   * @returns {boolean} True when the path is a supported script source file.
   */
  isScriptEntryPath(relativePath) {
    return (
      TsCompileFilter.SCRIPT_ENTRY_PATTERN.test(relativePath) &&
      !TsCompileFilter.TYPESCRIPT_DECLARATION_PATTERN.test(relativePath)
    );
  }
   ...
   *
   * @param {string} relativePath - Path relative to the output `BP/scripts` root.
   *
...
module.exports = TsCompileFilter;
