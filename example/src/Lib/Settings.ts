...
export default abstract class Settings {
    ...
    static readonly DEFAULT_REPEAT = 1;

    /** Highest repeat count the filter accepts. */
    static readonly MAX_REPEAT = 10;
     ...
     */
    static resolve(rawJson: string): ExampleSettings {
    ...
    static parse(rawJson: string): Record<string, unknown> {
        ...
        try {
            parsed = JsonTools.parse(rawJson);
        } catch (error) {
     ...
     */
    static readOptionalCount(settings: Record<string, unknown>, key: string, fallbackValue: number): number {
