...
import PsdMetadataReader from "./PsdMetadataReader";
import { MARKETING_ART_SPECS, REQUIRED_ROLES_BY_TYPE, STORE_ART_SPECS } from "../Data/AssetRequirements";
import type { ArtAsset, ArtGroup, ArtSpec, ArtSpecMap, ImageMetadata, ResolvedArgs } from "../Types/PackagerTypes";
...
export default class AssetValidator {
     ...
     */
    static async validate(args: ResolvedArgs): Promise<string[]> {
        const errors: string[] = [];
     ...
     */
    static validateArtCount(errors: string[], groupName: string, artGroup: ArtGroup, specs: ArtSpecMap, requiredRoles: string[]): void {
     ...
     */
    static async validateArtGroup(groupName: string, artGroup: ArtGroup, specs: ArtSpecMap, requiredRoles: string[]): Promise<string[]> {
        ...
        const validationTasks: Promise<string[]>[] = [];
        ...
        for (const roleKey of roleKeyList) {
            ...
            validationTasks.push(
                this.validateSingleAsset(
                    groupName,
                    roleKey,
                    artGroup[roleKey as keyof ArtGroup] as ArtAsset | undefined,
                    specs[roleKey],
                    null
                )
            );
     ...
     */
    static async readAssetMetadata(asset: ArtAsset): Promise<ImageMetadata> {
     ...
     * @returns Normalized metadata object.
     */
    static normalizeRasterMetadata(metadata: sharp.Metadata): ImageMetadata {
     ...
     */
    static matchesExpectedDpi(actualDpi: number, expectedDpi: number): boolean {
     ...
     */
    static formatDpi(metadata: ImageMetadata): string {
     ...
     */
    static roundDpi(dpi: number | undefined): string {
     ...
     */
    static resolveFormat(metadata: ImageMetadata, sourcePath: string): string {
     ...
     */
    static describeRoleKey(roleKey: string, index: number | null): string {
