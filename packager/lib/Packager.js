...
const AssetValidator = require("./AssetValidator.js");
const JsonTools = require("./JsonTools.js");
const PackManifest = require("./PackManifest.js");
const { CONTENT_TYPE_PACK_RULES } = require("../data/AssetRequirements.js");
...
class Packager {
   ...
   *
   * @param {string} cwd - Current Regolith workspace path.
   ...
   *
   * @param {string} rawJson - Raw filter settings JSON.
   *
   * @returns {Promise<{ gameFilePath: string; submissionFilePath: string }>} Generated output file paths.
   */
  async run(rawJson) {
   ...
   *
   * @returns {string[]} Pack validation error messages.
   */
   ...
   * @param {string | null} packPath - Candidate pack directory path.
   * @param {string} packLabel - Human-readable pack label.
   */
   ...
   * @param {object} args - Validated packager arguments.
   *
   * @returns {object} Archive source state used for archive generation.
   */
  stage(args) {
   ...
   *
   * @param {string | null} sourcePath - Absolute source directory path.
   * @param {object} args - Validated packager arguments.
   * @param {string} packKind - Pack kind identifier.
   *
   * @returns {{ filePathList: string[]; manifestContent: string | null; packReference: { pack_id: string; version: [number, number, number] } | null; sourcePath: string } | null} Pack source metadata.
   */
  createPackSourceState(sourcePath, args, packKind) {
    ...
    const manifestState = this.resolvePackManifestState(sourcePath);
    ...
    return {
      filePathList: this.listFilesRecursive(sourcePath, this.createExcludedPackFilePathSet(args, packKind)),
      manifestContent,
      packReference,
      sourcePath
    };
   ...
   *
   * @param {string} buildTempPath - Absolute temporary build output path.
   * @param {object} args - Validated packager arguments.
   *
   * @returns {{ gameOutputPath: string; gameTempPath: string; submissionOutputPath: string; submissionTempPath: string }} Archive path state.
   */
   ...
   *
   * @param {object} stageState - Archive source state.
   * @param {string} outputPath - Absolute submission zip output path.
   * @param {object} args - Validated packager arguments.
   * @returns {Promise<string>} Absolute submission zip path.
   */
   ...
   *
   * @param {object} stageState - Archive source state returned by `stage()`.
   * @param {string} outputPath - Absolute game package output path.
   * @param {object} args - Validated packager arguments.
   *
   * @returns {Promise<string>} Absolute game package file path.
   */
   ...
   *
   * @param {import("archiver")} archive - Active archive instance.
   * @param {object} args - Validated packager arguments.
   */
    ...
    if (Array.isArray(args.art.store.screenshots)) {
      for (let index = 0; index < args.art.store.screenshots.length; index += 1) {
        await this.appendArtAsset(
          ...
          this.joinArchivePath(
            "Store Art",
            AssetOutputNames.createStoreScreenshotName(args.storeAssetContentName, index)
          )
        );
      }
    }
   ...
   *
   * @param {import("archiver")} archive - Active archive instance.
   * @param {{ fileName: string; sourcePath: string }} asset - Asset source metadata.
   * @param {string} archivePath - Archive-relative output path.
   */
   ...
   *
   * @param {import("archiver")} archive - Active archive instance.
   * @param {{ filePathList: string[]; manifestContent: string | null; packReference: { pack_id: string; version: [number, number, number] } | null; sourcePath: string } | null} packState - Pack source metadata.
   * @param {string} archiveRootPath - Archive-relative pack root path.
   */
   ...
   * @param {import("archiver")} archive - Active archive instance.
   * @param {object} stageState - Archive source state returned by `stage()`.
   * @param {string} archiveRootPath - Archive-relative world-template root path.
   */
   ...
   *
   * @param {import("archiver")} archive - Active archive instance.
   * @param {{ pack_id: string; version: [number, number, number] } | null} packReference - World pack reference entry.
   * @param {string} archivePath - Archive-relative JSON file path.
   */
   ...
   * @param {string} packKind - Pack kind identifier.
   *
   * @returns {Set<string> | null} Normalized excluded file paths.
   */
   ...
   * @param {string} packKind - Pack kind identifier.
   *
   * @returns {boolean} `true` when the pack kind is a world-linked BP or RP.
   */
   ...
   *
   * @param {string} rootPath - Absolute source directory path.
   * @param {Set<string> | null} excludePathSet - Normalized relative file paths to skip.
   *
   ...
   *
   * @param {string} value - Candidate archive path.
   *
   ...
   *
   * @returns {string} Joined archive path.
   */
   ...
   *
   * @param {string} archivePath - Archive-relative path.
   *
   * @returns {boolean} `true` when the file extension is already compressed.
   */
   ...
   *
   * @param {string} sourcePath - Temporary built archive path.
   * @param {string} destinationPath - Final archive destination path.
   */
   ...
   *
   * @param {string} sourcePath - Temporary built archive path.
   * @param {string} destinationPath - Destination path in the output directory.
   */
   ...
   *
   * @param {string} destinationPath - Final artifact path.
   * @param {string} label - Label describing the temporary file role.
   *
   * @returns {string} Unique sibling temp file path.
   */
   ...
   *
   * @param {string | null} filePath - Temporary artifact path.
   */
   ...
   *
   * @param {string} outputPath - Absolute archive output path.
   * @param {(archive: import("archiver")) => Promise<void> | void} archiveBuilder - Callback that populates the archive.
   */
   ...
   *
   * @param {string[]} assetErrors - Asset validation failures.
   * @param {string[]} packErrors - Pack validation failures.
   *
   ...
   *
   * @param {string | null} directoryPath - Candidate directory path.
   *
  ...
  /**
   * Determines whether a path points to an existing file.
   *
   * @param {string | null} filePath - Candidate file path.
   *
   ...
   *
   * @param {string} packPath - Absolute pack directory path.
   *
   * @returns {{ actualPath: string | null; expectedPath: string; status: "case_mismatch" | "exact" | "missing" }} Manifest lookup result.
   */
