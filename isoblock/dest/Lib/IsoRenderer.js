...
const TgaDecoder_1 = __importDefault(require("./TgaDecoder"));
/**
 * Renders isometric PNG images of Minecraft Bedrock block models.
 */
class IsoRenderer {
    static AMBIENT_INTENSITY = 0.6;
    static SUN_INTENSITY = 0.8;
    // Inventory icons view the block from the north-east, showing its top,
    // north, and east faces.
    static CAMERA_DIRECTION = new THREE.Vector3(2, 1, -2).normalize();
    static BLOCK_CENTER = [0, 8, 0];
    static BLOCK_SIZE = 16;
    static DEFAULT_ISO_SCALE = 12;
     ...
     */
    async render(parts, outputPath, resolution = IsoRenderer.DEFAULT_RESOLUTION, modelRotation = null) {
        const gl = (0, gl_1.default)(resolution, resolution);
        gl.getInternalformatParameter = () => { };
        const renderer = new THREE.WebGLRenderer({
            context: gl,
            canvas: { addEventListener: () => { }, style: {}, getContext: () => gl },
            antialias: false,
        });
     ...
     */
    applyGeoRotation(object, [rx, ry, rz]) {
     ...
     */
    applyWorldRotation(object, [rx, ry, rz]) {
        object.rotation.set(THREE.MathUtils.degToRad(rx), THREE.MathUtils.degToRad(ry), THREE.MathUtils.degToRad(rz), "ZYX");
    }
    /**
     ...
     */
    computeFaceRects(cube, boneMirror) {
     ...
     * @returns UV rectangles keyed by face name.
     */
    createBoxUvRects(cube, boneMirror) {
        const [u, v] = cube.uv;
     ...
     */
    createFaceUvCoords(faceRect, geoTW, geoTH, frameV) {
        const { rect, rotation } = faceRect;
        const u = (x) => x / geoTW;
        const v = (y) => 1 - (y / geoTH) * frameV;
        let corners = [
            [u(rect[0]), v(rect[1])],
            [u(rect[2]), v(rect[1])],
            [u(rect[0]), v(rect[3])],
    ...
    createTextureLoader() {
        const cache = new Map();
        return async (texturePath) => {
            const cached = cache.get(texturePath);
            if (cached) {
                return cached;
            }
            const image = this.decodeImage(texturePath, await promises_1.default.readFile(texturePath));
     ...
     */
    decodeImage(texturePath, file) {
        if (texturePath.toLowerCase().endsWith(".tga")) {
            return TgaDecoder_1.default.decode(file);
        }
        return pngjs_1.PNG.sync.read(file);
    }
    /**
     ...
     */
    createMaterial(renderMethod, texture) {
        const params = { map: texture };
     ...
     */
    async buildCube(cube, bone, bonePivot, geoSpace, textureConfig, loadTexture) {
        ...
        for (const [faceName, faceRect] of Object.entries(rects)) {
            ...
            geometry.setAttribute("uv", new THREE.Float32BufferAttribute(this.createFaceUvCoords(faceRect, geoSpace.width, geoSpace.height, frameV), 2));
     ...
     */
    resolveFaceConfig(cube, faceName, textureConfig) {
     ...
     */
    isBoneHidden(bone, bonesByName, boneVisibility) {
        const visited = new Set();
        for (let current = bone; current && !visited.has(current.name); current = bonesByName[current.parent ?? ""]) {
     ...
     */
    getGeometryDefinition(modelData) {
        return modelData["minecraft:geometry"]?.[0] ?? Object.values(modelData)[0];
    }
    /**
     ...
     */
    async buildPart(part, loadTexture) {
     ...
     */
    createCamera(sceneRoot, multiBlock) {
     ...
     * @param outputPath - Destination .png path.
     */
    writePixelsToPng(gl, resolution, outputPath) {
        ...
        const png = new pngjs_1.PNG({ width: resolution, height: resolution });
        ...
        (0, fs_1.writeFileSync)(outputPath, pngjs_1.PNG.sync.write(png));
    }
}
exports.default = IsoRenderer;
